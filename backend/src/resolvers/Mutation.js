...
const Mutations = {
  async createItem(parent, args, ctx, info) {
    ...
    const item = await ctx.db.mutation.createItem(
      {
        data: {
          // create relationship Item-User by doing this:
          user: {
            connect: {
  ...
  async signin(parent, {email, password}, ctx, info) {
    ...
    const token = jwt.sign({ userId: user.id }, process.env.APP_SECRET);
    // 4. Set the cookies with the token
    ctx.response.cookie('token', token, {
      httpOnly: true,
  ...
  async resetPassword(parent, args, ctx, info) {
    ...
    const password = await bcrypt.hash(args.password, 10);
    // 5. Save the new password and remove the old reset Token fields
  ...
  async updatePermissions(parent, args, ctx, info) {
    ...
    // 4. if so, Update the permissions
    return ctx.db.mutation.updateUser(
      {
        data: {
          permissions: {
            //because using enum permissions use 'set'
  ...
  async removeFromCart(parent, args, ctx, info) {
    ...
    if(cartItem.user.id !== ctx.request.userId) {
      throw new Error ("Either not logged in or item not in current user's cart");
    }
